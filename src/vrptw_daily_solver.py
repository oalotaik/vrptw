...
def load_time_matrix(time_matrix_path: str) -> pd.DataFrame:
    ...
    assert (tm.index.tolist() == tm.columns.tolist()), "time_matrix.csv must be square with same ordered location labels on index and columns"
    return tm

def load_time_windows(path: Optional[str], locations: List[str]) -> Dict[str, Tuple[int,int]]:
    if path is None or not Path(path).exists():
        # Wide windows by default: [0, 9h]
        nine_h = 9 * 60
        return {loc: (0, nine_h) for loc in locations}
    tw = pd.read_csv(path)
    assert {"location","start","end"}.issubset(tw.columns), "time_windows.csv must have columns: location,start,end"
...
def build_day_instance(
    ...
    data = {
        ...
        "time_windows": [time_windows_map[loc] for loc in locations_today],  # list aligned to locations_today
        "time_matrix": tm_sub.values.astype(int).tolist(),
        "shift_minutes": 9 * 60,
        "vehicle_fixed_cost": vehicle_fixed_cost,
        "max_vehicles": (max_vehicles if max_vehicles is not None else N),  # upper bound: each stop could be its own route
        "day": day,
    }
    return data
...
def solve_vrptw(data: dict, time_limit_s: int = 30) -> dict:
    ...
    for v in range(data["max_vehicles"]):
        ...
        node = manager.IndexToNode(index)
        t = solution.Value(time_dim.CumulVar(index))
        r["stops"].append({"node": "DEPOT" if node == depot else locs[node], "start_time": int(t)})
        routes.append(r)
...
def main():
    ...
    ap.add_argument("--day", required=True, choices=DAYS, help="Day to solve (SAT..THU)")
    ap.add_argument("--schedule", default="schedule.csv")
    ap.add_argument("--time_matrix", default="time_matrix.csv")
    ap.add_argument("--time_windows", default=None, help="Optional CSV with columns: location,start,end (minutes)")
    ap.add_argument("--max_vehicles", type=int, default=None, help="Upper bound on vehicles (default: #stops)")
    ap.add_argument("--vehicle_fixed_cost", type=int, default=10**6)
    ap.add_argument("--time_limit", type=int, default=30)
    ap.add_argument("--out_prefix", default=None, help="Prefix for outputs; default: routes_{DAY}")
    args = ap.parse_args()
    ...
    if missing_in_matrix:
        raise ValueError(f"These schedule locations are missing from time_matrix: {sorted(missing_in_matrix)[:10]} ...")
